...
router.get('/', async (req, res) => {
  ...
  try {
    const experienceArr = await ExperienceModel.find({
...
router.get('/:id', async (req, res) => {
  ...
  try {
    const experienceArr = await ExperienceModel.findById(req.params.id).exec();
    res.status(200).send(experienceArr);
  } catch (e) {
    res.status(400).send(e);
  }
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const isUpdated = !!(await ExperienceModel.updateOne(
      { _id: req.params.id },
      updatedExperienceObj
    ).exec());
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    const isDeleted = !!(await ExperienceModel.deleteOne({
      _id: req.params.id,
    }).exec());
